...
class DistillationTrainer:
    ...
    def do_epoch(self, device, train=True):
        ...
        for i, (images, labels) in tqdm(enumerate(loader)):
            ...
            with torch.set_grad_enabled(train):
                ...
                dico['loss_features'] += loss_features.item()
    ...
    def logit_distillation_loss(self, logits_teacher, logits_student, T=5):
    ...
    def train_model(self, num_epochs, device):
        ...
        for epoch in range(num_epochs):
            ...
            print(
                f'TRAIN: Epoch[{epoch + 1}/{num_epochs}], Loss_Classif:{train_dico["loss_classif"]:.4f}, Loss_Logits:{train_dico["loss_logits"]:.4f},, Loss_features:{train_dico["loss_features"]:.4f},  Accuracy:{train_dico["accuracy"]:.4f}')


            test_dico = self.do_epoch(device, train=False)
            print(f'TRAIN: Epoch[{epoch + 1}/{num_epochs}], Loss_Classif:{test_dico["loss_classif"]:.4f}, Loss_Logits:{test_dico["loss_logits"]:.4f}, , Loss_features:{test_dico["loss_features"]:.4f}, Accuracy:{test_dico["accuracy"]:.4f}')
